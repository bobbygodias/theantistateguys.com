...
const displayPolygon = '185,127 560,112 560,151 185,170';
...
for (const geometry of geometries){
  ...
  for (const route of routes){
    ...
    const measured = await page.evaluate(() => {
      ...
      const screenPatch = document.querySelector('.boombox-source-patch');
      ...
      return {
        ...
        screenPatchReady:!!screenPatch && screenPatch.getAttribute('src')?.includes('canon/boombox-display-source-patch.webp'),
    ...
    if(route === 'home' && !measured.screenPatchReady) issues.push('radio-display-source-patch');
