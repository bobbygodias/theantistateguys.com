...
await page.waitForSelector('#play-track');
...
assert.equal(await page.locator('#insert-cd,#eject-cd,.cd-tray,.cd-disc,.cd-closed-panel,#cd-close-sfx').count(),0);
...
assert.equal(nativeGeometry.count,5);
...
const result={passed:true,media,nativeGeometry,checks:['only official slogan','no provisional notice','always-ready five-button native transport','no legacy CD drawer/SFX','official YouTube access in header','history 2024','six actual members and biographies','contact direct Web3Forms submission and official email'],errors};
