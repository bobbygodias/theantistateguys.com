...
function syncDisplay() {
  ...
  if (desktopCurrent)
    desktopCurrent.textContent = formatTime(audio.currentTime);
...
async function loadMusic() {
  try {
    ...
    tracks = releases.flatMap((r) =>
      (r.tracks || []).map((t) => ({ ...t, release: r.title || "" })),
    );
...
audio.addEventListener("error", () =>
  showToast("Não foi possível reproduzir esta faixa."),
);
...
function setRoute(name, { replace = false, focusHeading = false } = {}) {
  ...
  document.body.dataset.activeRoute = route.dataset.route;
  resetScroll();
...
homeFinalScript.src = "home-final-v1.js?rev=20260919-1";
...
contactFinalScript.src = "contact-final-v1.js?rev=20260916";
